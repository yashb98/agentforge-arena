...
async def save_message(room_id: int, username: str, content: str) -> MessageResponse:
    ...
    return MessageResponse(id=row["id"], room_id=row["room_id"], username=row["username"], content=row["content"], created_at=str(row["created_at"]))
...
async def get_messages(room_id: int, limit: int = 50, offset: int = 0) -> PaginatedMessages:
    ...
    messages = [MessageResponse(id=r["id"], room_id=r["room_id"], username=r["username"], content=r["content"], created_at=str(r["created_at"])) for r in rows]
...
async def search_messages(room_id: int, keyword: str) -> list[MessageResponse]:
    """Search messages by keyword within a room."""
    db = await get_db()
    rows = await (await db.execute(
        "SELECT * FROM messages WHERE room_id = ? AND content LIKE ? ORDER BY created_at DESC",
        (room_id, f"%{keyword}%"),
    )).fetchall()
    return [MessageResponse(id=r["id"], room_id=r["room_id"], username=r["username"], content=r["content"], created_at=str(r["created_at"])) for r in rows]
