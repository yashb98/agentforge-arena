...
## Key Modules
- `src/docker/manager.py` — SandboxManager: create, destroy, resource monitoring; seeds `project/.claude/settings.json` (`defaultMode: bypassPermissions` + broad allow — sandbox is the boundary), `project/.claude/rules/00-research-before-implement.md`, bundled `.claude/skills/`, and code-review-graph `project/.mcp.json` + `.code-review-graphignore`
