...
- First time setting up the graph for this team project
- After major refactoring or branch switches
- If the graph seems stale or out of sync
- The graph auto-updates via hooks on edit/commit when configured, so manual builds are rarely needed

## Notes

- The graph is stored as a SQLite database (`.code-review-graph/graph.db`) in this project root
