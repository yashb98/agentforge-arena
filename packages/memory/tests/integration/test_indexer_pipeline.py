...
class TestIndexingPipeline:
    ...
    @pytest.mark.asyncio
    async def test_index_empty_list_is_noop(self, pipeline, mock_semantic_store) -> None:
        """index_files([]) should not call upsert."""
        await pipeline.index_files([])
        mock_semantic_store.upsert.assert_not_called()

    @pytest.mark.asyncio
    async def test_remove_deleted_files(self, pipeline, mock_semantic_store) -> None:
